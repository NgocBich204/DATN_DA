...
@st.cache_data(ttl=600)  # ✅ CACHE 10 phút - chỉ load 1 lần
def load_customer_data_from_db():
    ...
    try:
        ...
        df['address'] = df['QuanHuyen'].fillna(
            'N/A') + ", " + df['TinhThanh'].fillna('N/A')
        df['region'] = np.select(
        ...
        # Tính toán
        df['last_purchase'] = df['LastPurchaseDate'].dt.strftime(
            '%d/%m/%Y').fillna('N/A')
        ...
        df['avg_order_value_str'] = df['avg_order_value'].apply(
            format_currency)
        ...
        df['status'] = np.select(
            [
                ...
                df['group_name'].str.contains(
                    'tiềm năng|mới', case=False, na=False),
                df['group_name'].str.contains(
                    'nguy cơ mất|chăm sóc|yếu', case=False, na=False),
            ],
            [
                'Rất quan trọng',
        ...
        df_orders['NgayMua'] = pd.to_datetime(
            df_orders['NgayMua'], errors='coerce')

        df_orders['Ngày Mua'] = df_orders['NgayMua'].dt.strftime(
            '%d/%m/%Y %H:%M:%S')
        df_orders['Thành Tiền'] = df_orders['DoanhThuThuan'].apply(
            format_currency)

        agg_orders = df_orders.groupby(['KhachHangID', 'MÃ ĐH', 'Ngày Mua', 'SL', 'Thành Tiền', 'Kênh'])[
            'Sản Phẩm'].apply(lambda x: ', '.join(x.astype(str).unique())).reset_index()
        ORDER_HISTORY_DATA = agg_orders[[
            'KhachHangID', 'MÃ ĐH', 'Ngày Mua', 'Sản Phẩm', 'SL', 'Thành Tiền', 'Kênh']]
        ...
        if 'ODBC Driver' in error_msg or 'Login failed' in error_msg:
            st.error(
                "Lỗi kết nối CSDL: Vui lòng kiểm tra tên Server, Database, hoặc Driver ODBC đã cài đặt.")
        else:
            st.error(f"Lỗi truy vấn CSDL: {error_msg}")
        return pd.DataFrame(), pd.DataFrame()


# ✅ HÀM CLEAR CACHE VÀ RELOAD DỮ LIỆU
def clear_cache_and_reload():
    """Xóa cache và tải lại dữ liệu từ DB"""
    load_customer_data_from_db.clear()
    st.rerun()
...
def filter_customers(search_term, from_date, to_date, region, channel, group, customer_data):
    ...
    is_date_filtered = (from_date != default_from_date) or (
        to_date != default_to_date)

    if is_date_filtered:
        try:
            df['registered_date'] = pd.to_datetime(
                df['registered'], format='%d/%m/%Y', errors='coerce')
...
def show_list_view():
    ...
    # ✅ LOAD DỮ LIỆU ĐÃ CACHE (chỉ query DB 1 lần)
    CUSTOMER_DATA, ORDER_HISTORY_DATA = load_customer_data_from_db()
    if CUSTOMER_DATA.empty:
        st.warning(
            "Không có dữ liệu khách hàng. Vui lòng kiểm tra kết nối CSDL và đảm bảo file ETL.py đã chạy thành công.")
        return

    # --- TITLE, SEARCH BAR VÀ BUTTON REFRESH ---
        ...
        with st.form("filter_form"):
            ...
            with cols[0]:
                from_date_input = st.date_input(
                    "Từ ngày", value=st.session_state.filter_from_date)
            with cols[1]:
                to_date_input = st.date_input(
                    "Đến ngày", value=st.session_state.filter_to_date)
            with cols[2]:
                unique_regions = ["Tất cả"] + \
                    list(CUSTOMER_DATA["region"].unique())
                region_index = unique_regions.index(
                    st.session_state.filter_region) if st.session_state.filter_region in unique_regions else 0
                selected_region_input = st.selectbox(
                    "Khu Vực", unique_regions, index=region_index)
            with cols[3]:
                unique_channels = ["Tất cả"] + \
                    list(CUSTOMER_DATA["fav_channel"].unique())
                channel_index = unique_channels.index(
                    st.session_state.filter_channel) if st.session_state.filter_channel in unique_channels else 0
                selected_channel_input = st.selectbox(
                    "Kênh Mua Hàng", unique_channels, index=channel_index)
            with cols[4]:
                unique_groups = ["Tất cả"] + \
                    list(CUSTOMER_DATA["group"].unique())
                group_index = unique_groups.index(
                    st.session_state.filter_group) if st.session_state.filter_group in unique_groups else 0
                selected_group_input = st.selectbox(
                    "Nhóm KH", unique_groups, index=group_index)

            with cols[5]:
                reset_pressed = st.form_submit_button(
                    "Xóa bộ lọc", use_container_width=True)
            with cols[6]:
                apply_pressed = st.form_submit_button(
                    "Áp dụng", type="primary", use_container_width=True)
    ...
    filtered_customers = filter_customers(
        ...
        CUSTOMER_DATA  # ✅ Truyền dữ liệu cached vào
    )
    ...
    if cols_nav[0].button("⬅️ ", disabled=(current_page == 0)):
        st.session_state.lookup_page -= 1
        st.rerun()

    if cols_nav[4].button(" ➡️", disabled=(current_page >= total_pages - 1)):
        st.session_state.lookup_page += 1
        st.rerun()
    ...
    if total_items > 0:
        cols_nav[2].write(
            f"Đang hiển thị **{start_display}** đến **{end_display}** (Trang **{current_page + 1}** / **{total_pages}**)")
    else:
...
def show_detail_view():
    ...
    try:
        ...
        order_history = ORDER_HISTORY_DATA[ORDER_HISTORY_DATA['KhachHangID'] == customer_id].drop(
            columns=['KhachHangID'], errors='ignore')
    ...
    with st.container(border=True):
        ...
        with cols[2]:
            ...
            st.markdown(
                f"**Ghi chú:** {get_badge_markdown(customer['status'])}")
            st.markdown(
                f"**Nhóm Khách Hàng:** {get_badge_markdown(customer['group'])}")
    ...
    with st.container(border=True):
        ...
        cols = st.columns(3)
        cols[0].metric("Recency (Ngày)",
                       f"{r_val}" if r_val != 'N/A' else "N/A")
        cols[1].metric("Frequency (Lần mua)", f"{f_val:.0f}" if isinstance(
            f_val, (int, float)) else "N/A")
        cols[2].metric("Monetary (Chi tiêu)", f"{m_val:,.0f} ₫".replace(
            ",", ".") if isinstance(m_val, (int, float)) else "N/A")
