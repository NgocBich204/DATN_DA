...
class ETLPipeline:
    ...
    def read_and_clean_excel(self, file_path):
        ...
        try:
            ...
            df['NgaySinh'] = pd.to_datetime(
                df['NgaySinh'], errors='coerce', dayfirst=True)
            df['NgayMua'] = pd.to_datetime(
                df['NgayMua'], errors='coerce', dayfirst=True)
            ...
            df['VanChuyen'] = pd.to_numeric(
                df['VanChuyen'], errors='coerce').fillna(0)
            df['DoanhThuThuan'] = pd.to_numeric(
                df['DoanhThuThuan'], errors='coerce')
    ...
    def calculate_rfm(self):
        ...
        try:
            ...
            df_rfm_final = df_rfm_grouped[[
                "KhachHangID", "Recency", "Frequency", "Monetary"]].copy()
    ...
    def kmeans_clustering(self, df_rfm, auto_k=True, manual_k=6):
        ...
        try:
            ...
            if auto_k:
                ...
                # Phương pháp Kneedle
                k_norm = (np.array(K_range) - min(K_range)) / \
            ...
            # Gán tên phân khúc
            cluster_summary = df_rfm.groupby(
            ...
            cluster_summary['R_Score'] = 1 - \
                (cluster_summary['Recency'] / r_max)
            ...
            cluster_ranking = cluster_summary.sort_values(
            ...
            if optimal_k == 4:
                ...
                ten_phan_khuc = ["Khách hàng VIP", "Khách hàng trung thành",
                                 "Khách hàng mới", "Khách hàng cần chăm sóc", "Khách hàng có nguy cơ mất"]
            elif optimal_k == 6:
                ten_phan_khuc = ["Khách hàng VIP", "Khách hàng trung thành", "Khách hàng tiềm năng",
                                 "Khách hàng mới", "Khách hàng cần chăm sóc", "Khách hàng yếu"]
            else:
                ten_phan_khuc = [f"Phân khúc {i+1}" for i in range(optimal_k)]

            phan_khuc_mapping = {cluster_id: ten_phan_khuc[idx] for idx, (
                cluster_id, row) in enumerate(cluster_ranking.iterrows())}
    ...
    def run_full_pipeline(self, file_path, mode="replace"):
        ...
        # 7. KMeans
        success, msg, segment_stats = self.kmeans_clustering(
            df_rfm, auto_k=True)
        if not success:
            self.close_db()
            return False, msg, stats
