...
config_data = {
    "objectives": [
        ...
        },
        {
            'id': 'awareness', 
            'icon': '🚀', 
            'title': 'Tăng Nhận diện và Tương tác',
            'description': 'Tái kích hoạt khách hàng ngừng mua để giảm tỷ lệ rời bỏ, tăng tương tác và khôi phục nguồn doanh thu tiềm năng .',
...
def send_email_campaign(campaign, email_list):
    ...
    try:
        if not hasattr(st, 'secrets') or 'email' not in st.secrets:
            st.error("❌ Chưa cấu hình email! Vui lòng tạo file .streamlit/secrets.toml với cấu hình email.")
            st.info("""
    ...
    if len(email_list) > 5:
        st.info(f"ℹ️ Giới hạn gửi 5 email demo. Tổng tệp: {len(email_list)} khách hàng.")
    ...
    try:
        ...
        st.error("❌ Lỗi xác thực email! Kiểm tra lại email/password trong secrets.toml")
        st.info("💡 Nếu dùng Gmail, hãy tạo App Password tại: https://myaccount.google.com/apppasswords")
        return False
...
def render_header_and_nav():
    ...
    with cols[1]:
        ...
        active_count = len([c for c in st.session_state.campaigns if c.get('status') == '🟢 Đang chạy'])
        st.button(
...
def render_step_1():
    ...
    with cols[0]:
        with st.container(border=True):
            ...
            st.write("Doanh nghiệp có Website, App, CRM riêng và muốn cá nhân hóa trải nghiệm trên các kênh này.")
            st.markdown("**Phù hợp với:** Email Marketing, Zalo OA, Web Popup, SMS...")
    ...
    with cols[1]:
        with st.container(border=True):
            ...
            st.write("Kinh doanh chủ yếu trên các sàn và muốn tận dụng công cụ của sàn giúp tăng trưởng .")
            st.markdown("**Phù hợp với:** Shopee, Lazada, Tiki (Ads, Voucher, Livestream...)")
            st.button("Chọn Sàn TMĐT", on_click=select_platform, args=('marketplace',), use_container_width=True, type="primary")
def render_step_2():
    st.button("⬅️ Quay lại", on_click=go_to_step, args=(1,), key="back_to_step1")
    if st.session_state.selected_platform == 'owned':
        ...
        st.write("📊 Hệ thống đã phân tích dữ liệu từ CSDL. Hãy xem hiện trạng và chọn mục tiêu ưu tiên.")
        with st.container(border=True):
            st.subheader("📈 Phân bố Phân khúc Khách hàng (Dữ liệu thực)")
            ...
            if df_real.empty:
                ...
                for i, (p, c, d) in enumerate(sorted_data):
                    with bar_cols[i]:
                        st.markdown(f"<div style='background-color:{c}; height: 15px; border-radius: 2px;' title='{d['PhanKhuc']}: {d['percentage']:.1f}%'></div>", unsafe_allow_html=True)
                
                legend_cols = st.columns(len(distribution))
                for i, (p, c, d) in enumerate(sorted_data):
                    with legend_cols[i]:
                        st.markdown(f"<span style='color:{c};'>●</span> **{d['PhanKhuc']}**: {d['percentage']:.1f}%", unsafe_allow_html=True)
                        st.caption(f"{d['SoLuong']} KH")
...
def render_step_3():
    ...
    with st.container(border=True):
        ...
        why_lines = [ln.strip() for ln in str(selected_obj.get('why', '')).splitlines() if ln.strip()]
    ...
    for _, segment in segments_to_show.iterrows():
        ...
        with st.container(border=True):
            # Header: Tên phân khúc (trái) + Nút chọn (phải) - CÙNG 1 HÀNG
...
def render_step_4():
    ...
    st.button("⬅️ Quay lại", on_click=go_to_step, args=(back_step,), key="back_to_step3_or_2")

    st.header("Bước 4: Đề xuất Kịch bản & Chiến thuật", divider="blue")
    st.write(f"🎯 Chọn kịch bản chiến thuật phù hợp cho **{st.session_state.selected_segment['name']}**")
...
def render_step_4_owned():
    ...
    st.caption("Tập trung vào cá nhân hóa sâu và tự động hóa trên website, app, CRM")
    ...
    for tactic in owned_tactics:
        ...
        with st.container(border=True):
            ...
            with cols[1]:
                ...
                st.markdown(f"**⏱️ Timeline:** {tactic.get('timeline', 'N/A')}")
...
def render_dynamic_form(tactic_id, tactic_type):
    st.subheader(f"⚙️ Cấu hình: {st.session_state.selected_tactic['name']}", divider="blue")
    ...
    if tactic_type == 'owned':
        ...
        st.markdown("---")
        st.text_area("📝 Nội dung tin nhắn", placeholder="Chào bạn,\n\nChúng tôi có ưu đãi...", key="tactic-email-body", height=150)
        
        col1, col2 = st.columns(2)
        with col1:
            st.text_input("🔘 Tiêu đề nút (CTA)", placeholder="Xem ngay!", key="tactic-email-button-text")
        with col2:
            st.text_input("🔗 Link nút", placeholder="https://shop.com/san-pham", key="tactic-email-button-url")
        ...
        if tactic_id == 'mp_ads':
            ...
            st.text_input("🔍 Từ khóa (phân cách bằng dấu phẩy)", placeholder="Vd: áo sơ mi, sơ mi công sở", key="tactic-ads-keywords")
            ...
            st.selectbox("🎟️ Loại Voucher", ["Giảm theo %", "Giảm theo số tiền", "Freeship"], key="tactic-voucher-type")
            cols = st.columns(2)
            ...
            st.text_area("📝 Kịch bản / Mô tả", key="tactic-live-script", height=120)
            st.text_input("🔗 Link sản phẩm ghim", placeholder="link1, link2,...", key="tactic-live-products")

        elif tactic_id == 'mp_combo':
            st.text_input("📦 Sản phẩm chính", placeholder="Vd: Áo sơ mi", key="tactic-combo-main")
            st.text_input("➕ Sản phẩm phụ", placeholder="Vd: Cà vạt", key="tactic-combo-sub")
            st.number_input("💰 Giá combo (VNĐ)", min_value=0, value=249000, key="tactic-combo-price")
        
        else:
            st.info("ℹ️ Chiến thuật này không cần cấu hình chi tiết.")

def render_step_5():
    st.button("⬅️ Quay lại", on_click=go_to_step, args=(4,), key="back_to_step4")
    ...
    with st.form(key="campaign_builder_form"):
        ...
        submitted = cols_btn[0].form_submit_button("💾 Lưu vào CSDL", use_container_width=True, type="primary")
        demo_clicked = cols_btn[1].form_submit_button("👁️ Xem Demo", use_container_width=True, type="secondary")
...
def render_demo_modal():
    ...
    def show_demo_dialog():
        ...
        # ==== KÊNH OWNED (WEB) ====
        if platform == 'owned':
            subject = dynamic_data.get('tactic-email-subject', 'Chưa có tiêu đề')
            image_data = dynamic_data.get('tactic-email-image', '')
            body = dynamic_data.get('tactic-email-body', 'Chưa có nội dung')
            button_text = dynamic_data.get('tactic-email-button-text', 'Xem ngay')
        ...
        # ==== KÊNH MARKETPLACE (SÀN TMĐT) ====
            ...
            tactic_id = st.session_state.selected_tactic.get('id', '') if st.session_state.selected_tactic else ''
            ...
            with st.container():
                ...
                if 'voucher' in tactic_id.lower():
                    ...
                    col1, col2 = st.columns(2)
                    with col1:
                        st.info(f"**Loại voucher:** {dynamic_data.get('tactic-voucher-type', 'N/A')}")
                        st.info(f"**Mã giảm giá:** {dynamic_data.get('tactic-voucher-code', 'N/A')}")
                    with col2:
                        st.success(f"**Giá trị:** {dynamic_data.get('tactic-voucher-value', 'N/A')}")
                        st.success(f"**Đơn tối thiểu:** {dynamic_data.get('tactic-voucher-min', 'N/A')} VNĐ")
                    st.warning(f"**Lượt dùng tối đa:** {dynamic_data.get('tactic-voucher-limit', 'N/A')} lượt")
                ...
                # Flash Sale
                    ...
                    col1, col2 = st.columns(2)
                    with col1:
                        st.info(f"**Sản phẩm:** {dynamic_data.get('tactic-fs-product', 'N/A')}")
                        st.success(f"**Giá sale:** {dynamic_data.get('tactic-fs-price', 'N/A')} VNĐ")
                    with col2:
                        st.warning(f"**Số lượng:** {dynamic_data.get('tactic-fs-limit', 'N/A')} sản phẩm")
                        st.info(f"**Thời gian:** {dynamic_data.get('tactic-fs-date', 'N/A')} {dynamic_data.get('tactic-fs-time', '')}")
                
                # Ads
                elif 'ads' in tactic_id.lower():
                    st.markdown("#### 📱 Thông tin Quảng Cáo")
                    st.info(f"**Sản phẩm quảng cáo:** {dynamic_data.get('tactic-ads-product', 'N/A')}")
                    st.success(f"**Từ khóa:** {dynamic_data.get('tactic-ads-keywords', 'N/A')}")
                    st.warning(f"**CPC mong muốn:** {dynamic_data.get('tactic-ads-cpc', 'N/A')} VNĐ")
                
                # Livestream
                elif 'livestream' in tactic_id.lower():
                    st.markdown("#### 📺 Thông tin Livestream")
                    st.info(f"**Tiêu đề:** {dynamic_data.get('tactic-live-title', 'N/A')}")
                    col1, col2 = st.columns(2)
                    with col1:
                        st.success(f"**Ngày:** {dynamic_data.get('tactic-live-date', 'N/A')}")
                    with col2:
                        st.success(f"**Giờ:** {dynamic_data.get('tactic-live-time', 'N/A')}")
                    if dynamic_data.get('tactic-live-script'):
                        st.text_area("Kịch bản:", dynamic_data.get('tactic-live-script', 'N/A'), height=100, disabled=True)
                
                # Combo
                elif 'combo' in tactic_id.lower():
                    st.markdown("#### 📦 Thông tin Combo Deal")
                    st.info(f"**Sản phẩm chính:** {dynamic_data.get('tactic-combo-main', 'N/A')}")
                    st.info(f"**Sản phẩm phụ:** {dynamic_data.get('tactic-combo-sub', 'N/A')}")
                    st.success(f"**Giá combo:** {dynamic_data.get('tactic-combo-price', 'N/A')} VNĐ")
                ...
                # Default
                    ...
                    for key, value in dynamic_data.items():
                        if key.startswith('tactic-') and value:
                            display_key = key.replace('tactic-', '').replace('-', ' ').title()
    ...
    for i, campaign in enumerate(campaigns):
        ...
        with cols[5]:
            if campaign_status == '📝 Đã lưu':
                st.button("▶️ Kích hoạt", key=f"act_{i}", on_click=activate_campaign, args=(campaign_id,), use_container_width=True)
                ...
                st.button("✏️ Xem/Sửa", key=f"res_{i}", on_click=show_result_modal, args=(campaign_id,), use_container_width=True)
    ...
    if 'editing_campaign_id' in st.session_state:
        ...
        if campaign:
            ...
            def show_result_dialog():
                st.write(f"Nhập doanh thu thực tế từ chiến dịch **{campaign['name']}** để tính toán ROI.")
...
if __name__ == "__main__":
    st.set_page_config(layout="wide", page_title="Module Chiến dịch Marketing", page_icon="🎯")
    show()
