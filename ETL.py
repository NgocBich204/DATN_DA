...
# Loại bỏ cột ẩn hoặc không cần thiết
df = df.loc[:, ~df.columns.str.contains("Unnamed")]

# Xóa bản ghi toàn null
...
# Convert kiểu số, lỗi convert thành NaN rồi loại bỏ dòng đó
...
# In số dòng còn lại sau làm sạch
...
# --- Ghi dữ liệu sạch vào StagingSaleData ---
for idx, row in df.iterrows():
    ...
    row.DonHang, row.NgayMua.date(), row.Traffic, row.TinhThanh, row.QuanHuyen,
...
# --- Tạo bảng Dimension & Fact (nếu chưa có hoặc xóa cũ để làm mới) ---
cursor.execute("""
...
# --- Insert dữ liệu vào các bảng dim và fact ---
...
SELECT HoTen, Email, SDT, GioiTinh, NgaySinh, TinhThanh, QuanHuyen, Traffic FROM (
    SELECT *, ROW_NUMBER() OVER (PARTITION BY Email ORDER BY NgayMua DESC) as rn FROM dbo.StagingSaleData
...
SELECT SKU, MIN(TenSanPham), MIN(NhaSanXuat), MIN(PhienBan) FROM dbo.StagingSaleData
...
USING (
    ...
    JOIN dbo.DimDate d ON sa.NgayMua = d.Ngay
) src
...
INSERT (DonHang, KhachHangID, SPID, NhomSPID, DateID, SoLuong, DoanhThu, TienKhuyenMai, DoanhThuThuan, VanChuyen)
VALUES (src.DonHang, src.KhachHangID, src.SPID, src.NhomSPID, src.DateID, src.SoLuong, src.DoanhThu, src.TienKhuyenMai, src.DoanhThuThuan, src.VanChuyen);
""")
conn.commit()

print("ETL cơ bản hoàn thành, tiến hành phân tích RFM ...")

# --- Bước 1-2: Lấy dữ liệu cho phân tích RFM từ DB ---
query = """
SELECT
    f.KhachHangID,
    f.DonHang,
